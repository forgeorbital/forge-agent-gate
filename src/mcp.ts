 ...
 *  - Write tools (place_order, cancel_order) are GATED: they run the local
...
const TOOLS: Tool[] = [
  ...
  },
  {
    name: "gate_status",
    description:
      "Report the active mandate summary and kill-switch state. Read-only; never trades.",
...
/** Build the MCP Server object around a ready GateEngine. */
export function buildMcpServer(gate: GateEngine, mandate: Mandate): Server {
  const server = new Server(
    { name: "com.forgeorbital/agent-gate", version: "0.1.0" },
    { capabilities: { tools: {} } },
  );
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        ...
        case "gate_status":
          return ok(mandateSummary(mandate, gate));
        default:
...
export async function startMcpServer(options: StartMcpOptions = {}): Promise<void> {
  ...
  const signatureValid = verifyMandateSignature(mandate);
  if (!signatureValid) {
    if (requireSigned) {
      throw new Error(
  ...
  console.error(
    `[forge-agent-gate] serving MCP over stdio | mandate=${mandate.mandateId} | venues=[${adapters
      .map((a) => a.venue)
      .join(", ")}] | recordMode=${config.forge.recordMode} | killEngaged=${gate.killEngaged()}`,
  );

  const server = buildMcpServer(gate, mandate);
